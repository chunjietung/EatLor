...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listFoodChoices = new ArrayList<String>(); //construct array list object
        adapter = new FoodListAdapter(MainActivity.this, R.layout.list_layout, listFoodChoices); //construct an adapter
        foodListView.setAdapter(adapter); //set the adapter for the food list
        context = this;
            ...
            public void onClick(View view) {
                ...
                if (editText.length() == 0)
                {
                    ...
                    new AlertDialog.Builder(context)
                            .setTitle("No Text") //set title name of dialog
                            .setMessage("Required to insert food choice") //set the message of the dialog
                            //display an  'ok' button in the dialog interface and set its OnClickListener
                            .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                                //call onClick method when the 'ok' button is clicked
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.cancel(); //dialog will dismiss
                                }
                            })
                ...
                else
                {
                    ...
                    listFoodChoices.add(editText.getText().toString()); //retrieve the text from the editText and add it into the list
        ...
        hitButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //random function will only run when more than 1 food choice is inserted into the list
                if(listFoodChoices.size() <= 1) {
                    //display alert dialog when user tries to random 1 or 0 food choice
                    new AlertDialog.Builder(context)
                            .setTitle("Insufficient food choice")   //set a title name to the dialog
                            .setMessage("Required at least 2 food choices to random") //set the message of the dialog
                            .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
